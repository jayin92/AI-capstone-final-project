...
class TSP_Env(gym.Env):
    def __init__(self, n, size = 1,seed = 0):
        super().__init__()
        ...
        random.seed(seed)
        self.action_space = (1)
        self.observation_space = ((self.n,self.n), (self.n, 2), (self.n))
    ...
    def reset(self):
        ...
        return self.state2vec(State(partial_solution=self.solution, W=self.dist_mat, coords=self.coords))
    ...
    def step(self, action):
        ...
        return self.state2vec(next_state), reward, done, info
    ...
    def state2vec(self, state):
        ...
        xv = np.array([[(1 if i in solution else 0),
